...
describe('POST /api/orders', () => {
  ...
  it('400 invalid_form on bad shipping (missing zip)', async () => {
    ...
    const res = await ordersPOST(makeJsonRequest('/api/orders', {
      method: 'POST',
      body: { ...SHIPPING, zip: '', selectedItemIds: [1] },
    }))
  ...
  it('400 invalid_form on bad zip regex', async () => {
    ...
    const res = await ordersPOST(makeJsonRequest('/api/orders', {
      method: 'POST',
      body: { ...SHIPPING, zip: 'abcde', selectedItemIds: [1] },
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('400 cart_empty when no cart at all', async () => {
    ...
    const res = await ordersPOST(makeJsonRequest('/api/orders', {
      method: 'POST',
      body: { ...SHIPPING, selectedItemIds: [1] },
    }))
    expect(res.status).toBe(400)
  ...
  it('409 insufficient_stock mapping', async () => {
    ...
    const item = await seedCartItem(cart.id, p.id, 5)
    const res = await ordersPOST(makeJsonRequest('/api/orders', {
      method: 'POST',
      body: { ...SHIPPING, selectedItemIds: [item.id] },
    }))
  ...
  it('mass assignment: total_cents/status/user_id from body are ignored', async () => {
    ...
    const res = await ordersPOST(makeJsonRequest('/api/orders', {
      method: 'POST',
      body: { ...SHIPPING, selectedItemIds: [item.id], total_cents: 0, status: 'shipped', user_id: 9999 },
    }))
    expect(res.status).toBe(200)
    const json = (await res.json()) as { id: number }
