...
describe('createOrderForUser', () => {
  it('happy path: stock decrements, snapshots persist, cart cleared, total computed', async () => {
    ...
    const cart = await getOrCreateCart(u.id)
    const item1 = await seedCartItem(cart.id, p1.id, 2)
    const item2 = await seedCartItem(cart.id, p2.id, 3)
  ...
  it('empty cart throws cart_empty; no order rows created', async () => {
    ...
    await expect(
      createOrderForUser(u.id, { ...SHIPPING, selectedItemIds: [1] }),
  ...
  it('insufficient stock on second item rolls back first decrement', async () => {
    ...
    const cart = await getOrCreateCart(u.id)
    const item1 = await seedCartItem(cart.id, p1.id, 2)
    const item2 = await seedCartItem(cart.id, p2.id, 5)

    await expect(
      createOrderForUser(u.id, { ...SHIPPING, selectedItemIds: [item1.id, item2.id] }),
  ...
  it('total overflow throws total_overflow (Number.isSafeInteger guard)', async () => {
    ...
    const cart = await getOrCreateCart(u.id)
    const item = await seedCartItem(cart.id, p.id, 2)
    await expect(
      createOrderForUser(u.id, { ...SHIPPING, selectedItemIds: [item.id] }),
  ...
  it('concurrent stock race: only one of two parallel orders for the last unit succeeds', async () => {
    ...
    const item1 = await seedCartItem(c1.id, p.id, 1)
    const item2 = await seedCartItem(c2.id, p.id, 1)

    const results = await Promise.allSettled([
      createOrderForUser(u1.id, { ...SHIPPING, selectedItemIds: [item1.id] }),
      createOrderForUser(u2.id, { ...SHIPPING, selectedItemIds: [item2.id] }),
    ])
